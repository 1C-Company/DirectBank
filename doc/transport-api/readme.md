...
+ [Порядок взаимодействия на уровне аутентификации](#1)
	+ [Аутентификация по логину и паролю](#1.1)

 		+ [Метод **Logon** (HTTP-метод POST)](#1.1.1)
 		
 		+ [Метод **LogonOTP** (HTTP-метод POST)](#1.1.2)

	+ [Рекомендации для банковского сервиса](#1.2)

+ [Настройка обмена электронными документами](#2)

	+ [Получение настроек обмена с банком в автоматическом режиме](#2.2)

		+ [Метод **GetSettings** (HTTP-метод POST)](#2.2.1)

	+ [Пример XML-файла настроек обмена с банком](#2.3)

+ [Порядок взаимодействия на транспортном уровне](#3)

	+ [Формирование и разбор транспортного контейнера](#3.1)

	+ [Отправка электронных документов из 1С](#3.2)

		+ [Метод **SendPack** (HTTP-метод POST)](#3.2.1)

	+ [Получение электронных документов в 1С](#3.3)

		+ [Метод **GetPackList** (HTTP-метод GET)](#3.3.1)

		+ [Метод **GetPack** (HTTP-метод GET)](#3.3.2)

+ [Проверка работоспособности обмена электронными документами](#4)

	+ [Пример XML-файла запроса-зонда](#4.1)

+ [Процедура синхронизации 1С с банковским сервисом](#5)
...
Аутентификация Клиента с целью создания сессии на сервисе Банка может проходить по логину и паролю + дополнительный OTP («one time password», опционально) ([Logon](#1.1.1)).


### <a name="1.1"></a> Аутентификация по логину и паролю



Система «1С:Предприятия 8» передает в Банк уникальный идентификатор Клиента в банковской системе (строка может содержать только ANSI-символы в соответствии с [RFC 2616](http://tools.ietf.org/html/rfc2616) для передачи значений в HTTP-заголовке) и строку, содержащую «логин + пароль» (строка «Basic логин:пароль» в формате BASE64 в соответствии с [Basic access authentication](http://tools.ietf.org/html/rfc2617)) (отправка производится HTTP-методом POST - метод [Logon](#1.1.1)).
...
## <a name="1.1.1"></a> Метод Logon (HTTP-метод POST) 
...
<ResultBank xmlns ="http://directbank.1c.ru/XMLSchema" formatVersion="2.1.2">
    <Success>
        <LogonResponse>
...
## <a name="1.1.2"></a> Метод LogonOTP (HTTP-метод POST)
...
### <a name="1.2"></a> Рекомендации для банковского сервиса
...
## <a name="2"></a> Настройка обмена электронными документами
...
- Банковская система формирует файл настроек обмена с банком (XML-файл, соответствующий [XML-схеме настроек обмена с банком](https://github.com/1C-Company/DirectBank/blob/master/doc/xsd-scheme/readme.md#1C-Bank_Settings)), содержащий параметры обмена данными с банком.
- Получение настроек проходит согласно протоколу, описанному в разделе [«Порядок взаимодействия на транспортном уровне»](#3).
...
<ResultBank xmlns ="http://directbank.1c.ru/XMLSchema" formatVersion="2.1.2">
    <Success>
		<GetSettingsResponse creationDate="2015-02-19T11:21:02" formatVersion="2.1.2" id="502036">
...
### <a name="2.3"></a>  Пример XML-файла настроек обмена с банком:
...
<Settings xmlns="http://directbank.1c.ru/XMLSchema"
    ...
    <Data>
        ...
        <FormatVersion>2.1.2</FormatVersion>
...
### <a name="3.1"></a> Формирование и разбор транспортного контейнера
...
### <a name="3.2"></a> Отправка электронных документов из 1С
...
- Аутентификация:
 -	Для аутентификации по логину и паролю, только [Logon](#1.1.1);
 -	Для аутентификации по логину и паролю с двухфакторной авторизацией — [Logon](#1.1.1), а затем — [LogonOTP](#1.1.2).
- Отправка транспортного контейнера с данными электронных документов из 1С — [SendPack](#3.2.1).


## <a name="3.2.1"></a> Метод SendPack (HTTP-метод POST)
...
<Packet xmlns="http://directbank.1c.ru/XMLSchema"
	...
	<Document id="a64225eb-9737-4d80-bd9d-1ffe5fdb63b1" dockind="10" formatVersion="2.1.2">
		<Data>
...
### <a name="3.3"></a> Получение электронных документов в 1С
...
- Аутентификация:
 - Для аутентификации по логину и паролю, только [Logon](#1.1.1);
 - Для аутентификации по логину и паролю с двухфакторной авторизацией — [Logon](#1.1.1), а затем — [LogonOTP](#1.1.2).
- Запрос на получение списка GUID транспортных контейнеров, готовых к отправке клиенту банком, — [GetPackList](#3.3.1).
- В цикле запрос на получение транспортного контейнера по GUID из ранее полученного списка [GetPack](#3.2.2)
...
## <a name="3.3.1"></a> Метод GetPackList (HTTP-метод GET)
...
## <a name="3.3.2"></a> Метод GetPack (HTTP-метод GET)
...
```
### <a name="4"></a> Проверка работоспособности обмена электронными документами.
...
- Если используется электронная подпись (см. раздел [«Обеспечение безопасности данных»](https://github.com/1C-Company/DirectBank/blob/master/doc/common-section/data-security.md#-Обеспечение-безопасности-данных)), то электронный документ «Извещение о состоянии электронного документа» подписывается.
...
#### <a name="4.1"></a> Пример XML-файла запроса-зонда
...
[Процедура отправки в банк](#3.2) выполняется в цикле. Для каждого подготовленного к отправке транспортного контейнера будут вызваны следующие методы:
- Аутентификация:
 -	Для аутентификации по логину и паролю, только [Logon](#1.1.1);
 -	Для аутентификации по логину и паролю с двухфакторной авторизацией — [Logon](#1.1.1), а затем — [LogonOTP](#1.1.2).
- Отправка транспортного контейнера с данными электронных документов из 1С — [SendPack](#3.2.1).
...
При [получении электронных документов из банковского сервиса в 1С](#3.3) будут последовательно вызваны следующие методы:
- Аутентификация:
 - Для аутентификации по логину и паролю, только [Logon](#1.1.1);
 - Для аутентификации по логину и паролю с двухфакторной авторизацией — [Logon](#1.1.1), а затем — [LogonOTP](#1.1.2).
- Запрос на получение списка GUID транспортных контейнеров, готовых к отправке клиенту банком — [GetPackList](#3.3.1).
- Для каждого GUID из ранее полученного списка запрос на получение транспортного контейнера — [GetPack](#3.3.2).
