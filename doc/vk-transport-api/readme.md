...
В случае когда возможностей [API обмена с банком из 1С](http://) недостаточно, можно использовать технологию обмена с банком из 1С через внешнюю компоненту.
...
Внешняя компонента разрабатывается по технологии Native API. В архиве с внешней компонентой должен находиться файл info.xml, описание формата файла см. в [приложении Б](http://). Для поддержки работы в Chrome обращения к методам ВК из БЭД производятся в асинхронном режиме см. `НачатьВызов<ИмяМетодаВК>.`
...
Более подробно см. [в приложении А](http://).
